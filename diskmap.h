...
namespace diskmap
{
	...
	public:
		map(std::istream &in, std::function<K(const V &)> check) : in_{in}, start_{in.tellg()}, check_{check} { }
		size_t size()
		{
			in_.seekg(start_);
			size_t ret;
			in_.read(reinterpret_cast<char *>(&ret), sizeof(size_t));
			return ret;
		}
		std::pair<bool, V> get(const K &query) // Pending the existence of std::optional...
		{
			...
			in_.seekg(binsearch(in_.tellg(), nelem, qhash));
			while (in_.tellg() >= start_ && readent().first == qhash) in_.seekg(-2 * skipsize, std::ios::cur);
			if (in_.tellg() < start_) in_.seekg(start_ + static_cast<std::streampos>(sizeof(size_t)));
			while (in_.tellg() < start_ + static_cast<std::streampos>(nelem * skipsize))
			{
		...
		{
			if (idx >= size()) throw std::runtime_error{"Out of bounds"}; // TODO Throw a range_error
