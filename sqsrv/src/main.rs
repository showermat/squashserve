...
use squashfs_ng::read::{Archive, Data, Dir, OwnedFile};
use thiserror::Error;
...
// probably in the squashfs_ng lib.
#[derive(Error, Debug)]
pub enum StartupError {
	...
	#[error("Error reading content from archive file: {0}")] Squashfs(#[from] squashfs_ng::SquashfsError),
...
pub enum AppError {
	...
	#[error("Path \"{0}\" contains invalid UTF-8")] PathUtf8(PathBuf),
	#[error("Failed to load library: {0}")] LibraryLoad(#[from] library::LoadError),
}
...
impl AppError {
	fn status(&self) -> Status {
		match self {
			...
			AppError::PathUtf8(_) => Status::BadRequest,
...
impl UncheckedPath {
	fn new() -> Self {
		Self { path: PathBuf::new(), original: PathBuf::new() }
	}

	fn decoded(&self) -> &Path {
		&self.path
	}

	fn encoded(&self) -> &Path {
		&self.original
	}
}
...
impl<'a> FromSegments<'a> for UncheckedPath {
	...
	fn from_segments(segments: Segments<'a>) -> std::result::Result<Self, Self::Error> {
		let mut dec = PathBuf::new();
		let mut enc = PathBuf::new();
		for segment in segments {
			...
			else if decoded.contains("/") { Err(SegmentError::BadChar('/'))?; }
			else {
				dec.push(&*decoded);
				enc.push(segment);
			}
		}
		Ok(Self { path: dec, original: enc })
	}
}
...
impl FromUriParam<UriPath, PathBuf> for UncheckedPath {
	...
	fn from_uri_param(path: PathBuf) -> UncheckedPath {
		UncheckedPath { original: library::path_encode(&path).expect("Non-UTF-8 path in URI"), path: path }
	}
}
...
impl UriDisplay<UriQuery> for QueryMap {
	fn fmt(&self, _f: &mut Formatter<UriQuery>) -> std::fmt::Result {
		unimplemented!();
	}
}
...
impl Config {
	...
	fn new() -> StartupResult<Self> {
		...
		let fallback: Vec<Box<dyn Fn() -> std::result::Result<PathBuf, std::env::VarError>>> = vec![
...
fn rsrc(app: State<App>, path: UncheckedPath) -> Result<Content<Vec<u8>>> {
	let (mut file, ctype) = app.resource(&path.decoded())?;
...
fn home(app: State<App>, remote_ip: Option<SocketAddr>) -> Result<Response> {
	...
	if !app.library().loaded("").expect("No default category") { app.library().load("")?; }
...
fn index<'a>(app: State<'a, App>, volume: &Volume, pathbuf: &PathBuf, mut dir: Dir) -> Result<Response<'a>> {
	let mut entries = dir.by_ref().take(INDEX_MAX).map(|child| {
		...
		Ok(IndexEntry {
			...
			mime: content_type(&child).map(|x| x.to_string()).unwrap_or("".to_string()),
			kind: name,
	...
	Ok(Response::html(app.template("index", &json!({"viewbase": app.viewbase(), "volume": volume.info().string_values(), "path": pathbuf, "tail": name, "entries": entries, "elided": elided_str}))?))
}
		...
		match vol.get(&pathbuf).map_err(HandleResult::ActualError)? {
			...
			Some(node) => match node.resolve().map_err(|e| HandleResult::ActualError(e.into()))? {
				...
				Some(resolved) => match resolved.data().map_err(|e| HandleResult::ActualError(e.into()))? {
					...
					_ => Err(HandleResult::ActualError(AppError::FileType(id, pathbuf, node.data().map(|x| x.name()).unwrap_or("(unknown)".to_string())))),
				},
			},
		}
	});
...
fn view<'a>(app: State<'a, App>, id: String, path: UncheckedPath, query: Option<QueryMap>) -> Result<Response<'a>> {
	match app.config.toolbar {
		true => {
			...
			let pathstr = path.decoded().as_os_str().to_str().expect("Non-UTF-8 request path").to_string();
						...
						[base, pattern, replacement] => match Regex::new(pattern) {
							Ok(re) => {
								let basestr = match base {
									"" => "".to_string(),
									b => b.to_string() + "/", // This is an unfortunate consequence of underthinking the format for specifying origins
								};
								Some(basestr + &re.replace_all(&pathstr, replacement))
							},
							Err(_) => None,
						},
						_ => None,
			...
			Ok(Response::html(app.template("view", &json!({"volume": info, "path": encoded_path, "query": querystr, "live": live}))?))
		},
		false => Ok(Response::redirect(uri!(content: id, path.decoded().to_path_buf()))),
	}
}

#[get("/view/<id>")]
fn view_default<'a>(app: State<'a, App>, id: String) -> Result<Response<'a>> {
	view(app, id, UncheckedPath::new(), None)
}
