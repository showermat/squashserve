...
Build requirements are as follows:
  ...
  - I have made no effort to develop code that will work on anything but Linux on an x86_64 architecture.  I may broaden this in the future.
...
The ZSR format is a general-purpose archive format designed to combine XZ compression with a random-access indexed scheme similar to that of ZIP.  It was designed specifically for this project, but there is no reason it could not be used for generic archiving as well.  The file format was designed with these aims:

  - **Fast random access**, necessary if the files in the archive are to be served by a web server.  This is accomplished by compressing files individually and storing the directory structure as an on-disk tree of hash maps.

  - **High compression**.  This was partially achieved by using XZ, which currently provides some of the best compression ratios available.  However, the effectiveness was decreased by the necessity of compressing each file separately to enable random access.  There is currently a modest improvement over ZIP, and I am looking for ways to further improve this ratio.

  - **Total encapsulation**, so that a volume can be transferred as a single file.  OpenZIM separates the search index from the ZIM file, which requires each user to index a file before it can be searched.  ZSR aims to encapsulate all data necessary to use the archive in a single file.
...
I've played around with support for full-content search with [Xapian](https://xapian.org/), but there have been numerous obstacles keeping it from being a practical solution.  The introduction of the *glass* backend, which can be stored as a single file embedded within an archive, was a big step toward making this practical.  The primary obstacle at this point is the size of the Xapian index, which is almost as large as the content archive in some cases, or half the size after aggressive text sanitization.  I will continue to explore this possibility; in the meantime, Xapian search works but is disabled by a compile-time flag.

In lieu of full-content search, a search on page titles has been built into zsrsrv.  During volume creation, the titles of all HTML files in the tree are indexed in a serialized radix tree that is stored in the user data portion of the volume, and this radix tree is traversed on-disk when a query is made.  This method has proven quite efficient and scalable, taking up almost no memory and requiring few disk reads.  Every title is indexed multiple times, once starting after each non-word character, so searches will find any page where the query is at the beginning of a word.

The first 40 results of the search are placed in a typeahead box below the search input.  Selecting one of these will take the user directly to the matching article.  Pressing enter will take the user to the matching article if there is an article whose title matches the query exactly, or if there is only one article matching the query.  Otherwise, it will display a page with a full list of all matching articles.
...
The biggest issue I encountered on the journey to my offline Wikipedia was obtaining a complete copy of the wiki's data for archiving.  It seems strange, but none of the options I have been able to find provide a fast, simple way to get a complete copy of all HTML and thumbnail images on the site.  Here is a summary of the methods I tried and how they worked out:
  ...
  - The simplest option, **spidering**, would take months for a site of this size.  Wikipedia does not forbid it, but states at <https://en.wikipedia.org/wiki/Wikipedia:Database_download#Why_not_just_retrieve_data_from_wikipedia.org_at_runtime.3F> that requests by robots should be made at a rate of less than one per second.  I immediately dismissed this as not viable.
  ...
  - The database dump pages on Wikimedia offer tantalizing links to static HTML dumps and image archives, but these have not been updated in ages, and I assume that I cannot count on their revival.
